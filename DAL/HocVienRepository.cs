    ...
    public class HocVienRepository : IHocVienRepository
    {
        ...
        public string Insert(HocVien obj)
        {
            using (IDbConnection db = new SqlConnection(Helper.ConnectionString))
            {
                ...
                p.AddDynamicParams(new { MaHV = obj.MaHV, TenHV = obj.TenHV, HoNV = obj.HoHV, NgaySinh = obj.NgaySinh, SoCMND = obj.SoCMND, NgayCapCMND = obj.NgayCapCMND,SDT=obj.SDT,DiaChi=obj.DiaChi, DiemTH=obj.DiemTH, NgayDK = obj.NgayDK , KiThi=obj.KiThi });
        ...
        public bool Update(HocVien obj)
        {
            ...
            { 
                if (db.State == ConnectionState.Closed)
                    db.Open();
                    int result =db.Execute("sp_HocVien_Update", new { MaHV = obj.MaHV, TenHV = obj.TenHV, HoNV = obj.HoHV, NgaySinh = obj.NgaySinh, SoCMND = obj.SoCMND, NgayCapCMND = obj.NgayCapCMND, SDT = obj.SDT, DiaChi = obj.DiaChi, DiemTH = obj.DiemTH, NgayDK = obj.NgayDK, KiThi = obj.KiThi }, commandType: CommandType.StoredProcedure);
                return result != 0;
            }
